...
TEST_CASE("Camera: Constructing a camera") {
...
TEST_CASE("Camera: The pixel size for a horizontal canvas") {
    const Camera c{200, 125, math_constants::pi_by_two<>};
    REQUIRE(ALMOST_EQUALS(c.getPixelSize(), 0.01));
}

TEST_CASE("Camera: The pixel size for a vertical canvas") {
    const Camera c{125, 200, math_constants::pi_by_two<>};
    REQUIRE(ALMOST_EQUALS(c.getPixelSize(), 0.01));
}

TEST_CASE("Camera: Construct a ray through the centre of the canvas") {
...
TEST_CASE("Camera: Construct a ray through a corner of the canvas") {
    const Camera c{201, 101, math_constants::pi_by_two<>};
...
TEST_CASE("Camera: Construct a ray when the camera is transformed") {
    const Camera c{201, 101, math_constants::pi_by_two<>,
...
TEST_CASE("Camera: Rendering a world with a camera") {
