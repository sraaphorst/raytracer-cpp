...
int main() {
    ...
    middle_sphere->setMaterial(Material{make_colour(0.1, 1, 0.5), Material::DEFAULT_AMBIENT, 0.7, 0.3, Material::DEFAULT_SHININESS});

    auto left_sphere = Sphere::createSphere();
    left_sphere->setTransformation(translation(-2, 0.33, -0.75) * scale(0.33, 0.33, 0.33));
    left_sphere->setMaterial(Material{make_colour(1, 0.8, 0.1), Material::DEFAULT_AMBIENT, 0.7, 0.3, Material::DEFAULT_SHININESS});

    auto right_sphere = Sphere::createSphere();
    right_sphere->setTransformation(translation(1.5, 0.5, -0.5) * scale(0.5, 0.5, 0.5));
    right_sphere->setMaterial(Material{make_colour(0.5, 1, 0.1), Material::DEFAULT_AMBIENT, 0.7, 0.3, Material::DEFAULT_SHININESS});
